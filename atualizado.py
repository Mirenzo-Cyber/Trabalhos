...
def cadastrar_ativo():
    ...
    # 3. Entrada de Dados e Decisão Lógica: Pede o tipo de ativo e tenta buscar o nome da categoria no Enum. Se não for válido, mostra mensagem de erro.
    try:
        ...
        nome_categoria = CategoriasAtivos(tipo_numero).name  # Tenta buscar o nome da categoria no Enum
    except ValueError:  
    ...
    ativos_ti[id_ativo] = {
        ...
        "setor": setor,
        "tipo": nome_categoria,
        "vulnerabilidades": []  # Lista vazia para preencher depois com vulnerabilidades
    }
...
def listar_ativo():
    ...
    # Criamos variáveis vazias para guardar o resultado da nossa busca
    ativo_encontrado = None
    id_ativo_encontrado = 0
    
    # 1. O if e else separam a busca por ID (número) ou Nome (texto). Se for número, convertemos para int e procuramos no dicionário. Se for texto, percorremos o dicionário procurando pelo nome.
    if busca.isdigit():
        id_busca = int(busca)
        ...
        for id_ativo, dados in ativos_ti.items():
            if dados['nome'] == busca:
                ...
                break # O break faz o 'for' parar de procurar assim que acha o nome
                
    # 2. Se a variável ativo_encontrado foi preenchida, mostramos os dados. Se não, mostramos uma mensagem de erro.
    if ativo_encontrado:
...
def cadastrar_vulnerabilidade():
    ...
    # 1. Pede o ID do ativo que possui a falha
    try:
    ...
    # 2. Verifica se o ativo realmente existe na memória
    if id_busca in ativos_ti:
        print(f"Ativo selecionado: {ativos_ti[id_busca]['nome']}")
        
        # 3. Coleta os dados da vulnerabilidade
        ...
        # 4. Agrupa os dados em um novo dicionário
        ...
        # 5. Adiciona a vulnerabilidade na lista daquele ativo específico
...
def atualizar_ativo():
    ...
    #2. Decisão Lógica: Verifica se o ID existe no dicionário
    if id_busca in ativos_ti:
        ativo = ativos_ti[id_busca]
        print(f"Ativo encontrado: {ativo['nome']}")

        #3. Entrada de Dados: Pede os novos dados
        ...
        #4. Processamento (memória): Atualiza os dados no dicionário
        ativos_ti[id_busca]['responsavel'] = novo_responsavel
        ativos_ti[id_busca]['setor'] = novo_setor

        #5. Processamento (arquivo): Atualiza os dados no arquivo de texto
        with open("ativos.txt", "w") as f:
            for id_ativo, dados in ativos_ti.items():
                vuls_formatadas = ", ".join([v['descricao'] for v in dados['vulnerabilidades']]) if dados['vulnerabilidades'] else "Nenhuma"
...
def excluir_ativo():
    ...
    #1. Entrada de Dados: Pede o ID do ativo que deseja excluir
    try:
    ...
    if id_busca in ativos_ti:
        ...
        #3. Processamento (memória): Remove o ativo do dicionário
        del ativos_ti[id_busca]

        #4. Processamento (arquivo): Reescreve o arquivo de texto com os arquivos que sobraram
        with open("ativos.txt", "w") as f:
            for id_ativo, dados in ativos_ti.items():
                vuls_formatadas = ", ".join([v['descricao'] for v in dados['vulnerabilidades']]) if dados['vulnerabilidades'] else "Nenhuma"
                f.write(f"{id_ativo},{dados['nome']},{dados['responsavel']},{dados['setor']},{dados['tipo']},{vuls_formatadas}\n")
...
#Laço de repetição para atualizar vulnerabilidade

def atualizar_vulnerabilidade():
    # 1. Pedimos o ID do ativo que terá a vulnerabilidade atualizada
    try:
    ...
    if id_busca in ativos_ti:
        ...
        # 2. Proteção adicionada para a escolha da vulnerabilidade
        try:
...
# Criamos a variável vazia apenas para o laço poder iniciar sem dar erro
opcao = ""

# Loop: O código abaixo vai se repetir ENQUANTO a opção digitada não for "7"
while opcao != "7":
